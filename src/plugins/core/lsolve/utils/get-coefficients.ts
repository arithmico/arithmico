...
function getSummandCoefficient(summand: SyntaxTreeNode) {
...
function getVariableNamesFromEquations(equations: Equals[], context: Context): string[] {
  return [...equations.reduce(
    (variableNames, equation) => new Set([
      ...variableNames,
      ...getVariableNames(equation, context)
    ]), new Set<string>()
  )];
}
